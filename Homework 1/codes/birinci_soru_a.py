...
learning_rate = 1;                # learning rate 1 olarak belirlenmiştir.
...
w = np.random.randint(-3,4,(7,1))           # (7,1) boyutunda ilk ağırlık matrisi: [-3,3] aralığında

np.random.seed(40)                 
fp_train_data=np.random.randint([-3,-3,0,2,-3,0,1,1],[0,4,4,4,-1,4,2,2],(15,8)) # 1. eğitim kümesi için 15 adet 8 boyutlu [-3,3] değer aralığında noktalar üretildi.
                                                                                # matrisin normalde (15,6) boyutunda olması gerekirdi
                                                                                # Ancak 1 boyut bias için 1 boyut da etiket için eklendi
                                                                                # Matris çarpımlarında etiket uzayı kullanılmayacak.                   

np.random.seed(40)   
sp_train_data=np.random.randint([1,-3,-3,-3,-1,-3,1,-1],[4,4,0,1,4,0,2,0],(15,8)) # 2. eğitim kümesi için 15 adet 8 boyutlu [-3,3] değer aralığında noktalar üretildi.
...
for i in range(iteration):                # iterasyon sayısı kadar döngü devam edecek
    ...
    for j in range(length):               # train_data daki eleman sayısı kadar döngü devam edecek
        ...
        v = np.dot(wt,xn)                   # matrix çarpımının sonucu v değişkenine atandı. 
    ...
    if(counter==length):break               # bütün elemanlar doğru sınıflandırıldıysa iterasyon sayısını tamamlamadan döngüden çık 
           
    
    
np.random.seed(50)                      # np.random.seed(40) yaparsak test kümesini, eğitim kümesindeki değerlerin aynısını ile oluşturmuş oluruz.  
fp_test_data=np.random.randint([-3,-3,0,2,-3,0,1,1],[0,4,4,4,-1,4,2,2],(10,8))    # 1. test kümesini oluşturma. 
                                                                                  # 1. eğitim kümesi ile aynı aralıkta değerler üretildi
np.random.seed(50) 
sp_test_data=np.random.randint([1,-3,-3,-3,-1,-3,1,-1],[4,4,0,1,4,0,2,0],(10,8))  # 2. test kümesini oluşturma  
                                                                                  # 2. eğitim kümesi ile aynı aralıkta değerler üretildi
...
for k in range(length):        # test kümesindeki bütün elemanlar için döngü
    ...
    xn=test_data_wl[:,k].reshape(7,1)  # test_data_wl matrisinde sıradaki eleman wt ile çarpılabilecek hale getirilir.
