...
#f(x1,x2,x3)= 0.5*x1*x2 + x2**2*e**-x3     xi, [0,1] aralığında

# bu fonksiyon adaline ile gerçeklenmek isteniyor. 
#(bizim oluşturduğumuz kümedeki x'ler weight ile çarpıldığında fonksiyonun sounucu ile yakın değerler vermeli)
...
def f_function(x1,x2,x3):                     # Yaklaşık olarak ifade edilmek istenilen fonksiyon tanımlanıyor.     
    return 0.5*x1*x2+x2**2*math.e**(-x3)      # fonksiyonun değeri döndürülecek
...
for i in range(length):             # data matrisindeki her elamanın ilk 3 sütun değeri, fonksiyonda yerine yazılmak için ayrıldı
        ...
        data[i][4]=f_function(x1,x2,x3)       # 4. sütun, fonksiyonun gerçek değerlerini belirtmek içindi.
                                              # fonksiyonun sonucu data matrisinin her elemanının 4. sütununa yazıldı
...
# Eğitimi durdurmak için iki yol
# Kümedeki elemanlardan, maksimum hataya sahip olan elemanın hatası belirli bir sınırın altına düşünce eğitim dursun
# Hataların ortalaması belirli bir sınırın altına inince eğitim durdurulur
# Başka bir yol kullanarak da eğitim belirtilen iterasyona gelmeden bitirilebilir
...
for i in range(iteration):                    # iterasyon sayısı kadar döngü devam edecek
    ...
    if(mean<0.00105): break;                  # hataların ortalamaso 0.00105 den küçükse eğitimi bitir.
